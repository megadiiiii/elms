        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/75 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700 text-[10px] font-black uppercase text-slate-400 dark:text-slate-300 tracking-[0.1em] transition-colors duration-300">
              ...
              <th className="p-4">{t("studentsLink")}</th>
              <th className="p-4">{t("contact")}</th>
              <th className="p-4">{t("studentNickname")}</th>
              <th className="p-4">{t("status")}</th>
              {localStorage.getItem("role") === "ADMIN" && <th className="p-4 pr-6 text-right">{t("actions")}</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr>
                <td colSpan={localStorage.getItem("role") === "ADMIN" ? "6" : "5"} className="p-4 text-center">
                  <LoadingSpinner />
                </td>
              </tr>
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <img
                           src={getAvatarSource(student.avatar)}
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-slate-800 dark:text-slate-100 truncate">{student.fullName}</p>
                          <p className="text-[10px] font-mono text-slate-400 dark:text-slate-300 font-bold tracking-tight mt-0.5 uppercase">
                            {student.studentCode || t("noCodeAssigned")}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="text-xs space-y-0.5">
                        <p className="text-slate-600 dark:text-slate-300 font-medium">{student.email}</p>
                      </div>
                    </td>
                    <td className="p-4 text-xs font-semibold text-slate-500">
                      {student.studentNickname || t("notUpdated")}
                    </td>
                    <td className="p-4">
                      ...
                      <td className="p-4 pr-6">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              onResetPassword(student.id, student.fullName);
                            }}
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              onToggleStatus(student.id, student.fullName, student.status);
                            }}
                            ...
                            title={student.status === "ACTIVE" ? t("lockAccount") : t("unlockAccount")}
                          >
              ...
              <tr>
                <td colSpan="6" className="p-16 text-center">
                  <div className="flex flex-col items-center gap-2 max-w-sm mx-auto">
                    ...
                    <p className="text-sm font-bold text-slate-700">{t("noStudentFound")}</p>
                    <p className="text-xs text-slate-400 leading-relaxed">
                      {t("noStudentFoundDesc")}
                    </p>
                  </div>
                </td>
              </tr>
      ...
      {!loading && totalPages >= 1 && (
        <div className="bg-slate-50/50 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-700 px-6 py-4 flex items-center justify-between transition-colors duration-300">
          <p className="text-xs text-slate-400 dark:text-slate-300 font-bold">
            {t("showingPage")} {currentPage} / {totalPages} ({t("totalUnit")} {totalElements} {t("studentsUnitSingular")})
          </p>
