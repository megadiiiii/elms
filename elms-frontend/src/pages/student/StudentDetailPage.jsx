...
const StudentDetailPage = () => {
    ...
    if (loading) {
        return (
            <MainLayout title={t("studentManagement")}>
        ...
        return (
            <MainLayout title={t("studentManagement")}>
                <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-slate-100 shadow-sm px-6">
                    ...
                    <h3 className="text-base font-bold text-slate-700">{t("studentProfileNotFound")}</h3>
                    <p className="text-xs text-slate-400 mt-2">{t("studentProfileNotFoundDesc")}</p>
                    <button
    ...
    return (
        <>
            <MainLayout
                ...
                description={t("studentDetailDesc").replace("{name}", detail.fullName)}
            >
                <div className="w-full bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden animate-fade-in">
                    ...
                    <div className="border-b border-slate-100 px-8 py-3.5 flex items-center justify-between bg-white">
                        ...
                        {isAdmin && (
                            <button
                                type="button"
                                onClick={() => navigate(`/admin/students/edit/${detail.id}`)}
                        ...
                        <div className="col-span-12 md:col-span-4 lg:col-span-3 flex flex-col items-center text-center space-y-5 md:border-r border-slate-100 md:pr-8 pt-4">
                            ...
                            <div className="space-y-2 w-full">
                                ...
                                <p className="text-[10px] font-mono text-slate-400 font-bold uppercase tracking-wider bg-slate-50 py-1 px-3 rounded-lg inline-block border border-slate-200/50">
                                    {detail.studentCode || t("noCodeAssigned")}
                                </p>
                            </div>

                            <div className="flex flex-col gap-2 w-full pt-2">
                                <div className="flex justify-between items-center text-xs py-1.5 border-b border-slate-50">
                                    <span className="text-slate-400 font-medium">{t("role") || "Chức vụ"}:</span>
                                    <StatusBadge type="role" value="STUDENT" />
                                </div>
                                <div className="flex justify-between items-center text-xs py-1.5">
                                    <span className="text-slate-400 font-medium">{t("status")}:</span>
                                    <StatusBadge type="user" value={detail.status} />
                                </div>
                            </div>
                                ...
                                <div className="flex flex-col gap-2 w-full pt-4 border-t border-slate-100/60">
                                    ...
                                    >
                                        <span className="material-symbols-outlined text-base">
                                            {detail.status === "ACTIVE" ? "lock" : "lock_open"}
                                        </span>
                                        {detail.status === "ACTIVE" ? t("lockAccount") : t("unlockAccount")}
                                    </button>
                                </div>
                            )}
                        </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-2 text-slate-700 font-semibold text-sm mt-1">
                                            <span className="material-symbols-outlined text-slate-400 text-[18px]">face</span>
                                            {detail.studentNickname || t("notUpdated")}
                                        </div>
                                    </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-2 text-slate-700 font-semibold text-sm mt-1">
                                            <span className="material-symbols-outlined text-slate-400 text-[18px]">supervisor_account</span>
                                            {detail.parentName || t("notUpdated")}
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-2 text-slate-700 font-semibold text-sm mt-1">
                                            <span className="material-symbols-outlined text-slate-400 text-[18px]">call</span>
                                            {detail.parentPhone ? (
                                                <a href={`tel:${detail.parentPhone}`} className="hover:text-blue-600 transition-colors">
                                                    {detail.parentPhone}
                                                </a>
                                            ) : (
                                                t("notUpdated")
                                    ...
                                    <div className="space-y-1 sm:col-span-2">
                                        ...
                                        <div className="flex items-center gap-2 text-slate-700 font-semibold text-sm mt-1">
                                            <span className="material-symbols-outlined text-slate-400 text-[18px]">mail</span>
                                            {detail.parentEmail ? (
                                                <a href={`mailto:${detail.parentEmail}`} className="hover:text-blue-600 transition-colors">
                                                    {detail.parentEmail}
                                                </a>
                                            ) : (
                                                t("notUpdated")
